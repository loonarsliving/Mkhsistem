-- ============================================================================
-- MK Connect — 0241: Group lump-sum bukti transfer by destination account,
-- not by submitter
--
-- Owner's ask: when a bukti transfer's nominal doesn't match any single
-- pending item, group by the destination bank account the money actually
-- landed in -- not by who submitted the pengajuan (a Kepala Cabang can
-- submit on someone else's behalf; the money still only ever lands in one
-- specific account).
--
-- Stores the pengajuan's own destination-account text (mkh-properti's
-- "keterangan" field, e.g. "BCA 6975175212 endy septianto") on
-- finance_pending_transfers at insert time so the confirm flow can compare
-- it against the AI-read rekeningTujuan from the bukti transfer photo.
-- ============================================================================

alter table public.finance_pending_transfers add column rekening_tujuan text;
