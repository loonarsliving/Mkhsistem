-- Owner's ask: Endy's bahan pengajuan currently notifies Vando for
-- verification the instant it's submitted, regardless of whether a nota
-- photo has actually been sent yet -- Vando can end up approving a
-- pengajuan with nothing to back it up. Hold that notification instead:
-- when no nota photo has arrived recently, queue what Vando's alert would
-- have said instead of sending it, and release it (to Vando) the moment
-- Endy's photo actually comes in and gets forwarded -- same photo now also
-- reaches Vando as evidence via the existing photo_auto_forward_rules path.
create table public.pending_expense_approval_notifications (
...
create index pending_expense_approval_notifications_employee_idx
  on public.pending_expense_approval_notifications (employee_id, created_at desc);

-- Called from photo-auto-forward.ts right after a sender's photo is
-- successfully forwarded -- releases every notification queued for that
-- employee by re-resolving the branch's CURRENT Kepala Cabang (not whoever
-- held the role at submission time) and deleting the queued row. A no-op
-- (returns 0) for any employee with nothing queued, so safe to call
-- unconditionally after every forwarded photo.
create or replace function public.release_pending_expense_approval_notifications(p_employee_id uuid)
returns integer
language plpgsql
security definer
...
begin
  for v_row in
    select * from public.pending_expense_approval_notifications
  ...
  loop
    if v_row.branch_id is not null then
      ...
      loop
        ...
        values (v_admin.id, 'system', 'finance_expense_pending_verification', v_row.title, v_row.body, v_row.link, v_row.metadata);
      end loop;
    end if;
    delete from public.pending_expense_approval_notifications where id = v_row.id;
    v_count := v_count + 1;
  end loop;
  return v_count;
end;
    ...
    if p_event_type = 'finance_payment_confirmed' then
      ...
      if p_payload ->> 'tipe' = 'bahan' and coalesce(p_payload ->> 'admin_email', '') ilike '%endy%' then
        ...
        if v_sender.id is not null then
          ...
          v_hold_for_photo := not v_has_recent_photo;
        end if;
      end if;

      if v_hold_for_photo then
        insert into public.pending_expense_approval_notifications (employee_id, branch_id, title, body, link, metadata)
        values (v_sender.id, v_branch_id, v_kc_title, v_kc_body, v_kc_link, v_kc_metadata);
      ...
      if v_hold_for_photo then
        ...
        values (
          ...
          '🧾 Anda mengajukan pembelian: ' || coalesce(p_payload ->> 'item', '-')
            ...
            || E'\n\n📸 Belum ada foto bahan yang diterima sebagai bukti/alasan pemesanan ini. Pengajuan ini masih DITAHAN, belum dikirim ke Kepala Cabang untuk verifikasi -- akan otomatis diteruskan begitu foto diterima.'
