-- ============================================================================
-- MK Connect — 0261: Fix stale-verification reminder outliving its own approval
--
-- Real incident (2026-09-17 through 2026-09-22): pengajuan 359 ("Cat
-- exterior 1 pail", Loonars Living) was submitted AND approved by Vando
-- within 21 seconds (a fast bulk-approve), but its
-- pengajuan_verification_reminders row never got cleaned up -- the
-- "Pengingat: Pengajuan Baru Menunggu Verifikasi" WhatsApp reminder kept
-- firing every ~2 hours for FIVE DAYS after it was already decided, even
-- though nothing was actually still pending.
--
-- Root cause: finance_expense_submitted (creates the reminder row) and
-- finance_expense_approved (deletes it) are two SEPARATE outbound sync
-- events dispatched via the shared sync_log queue -- nothing guarantees
-- they're delivered to sync_inbound in submission order. When approval
-- happens fast enough, the DELETE can run before the INSERT even exists,
-- so the INSERT (which fires moments later) recreates a reminder row for
-- something that was already resolved, and nothing ever removes it again.
--
-- Two-layer fix:
-- 1. finance_expense_submitted no longer inserts a reminder row at all if
--    a finance_pending_transfers row already exists for that pengajuan_id
--    -- the reliable, order-independent signal that it was already
--    approved, regardless of which event arrived first.
-- 2. remind_stale_pengajuan_verification() self-heals as a second line of
--    defense: before nagging about a row, it deletes (and skips reminding
--    for) any row whose pengajuan already has a finance_pending_transfers
--    entry, in case a similar race slips through some other path in the
--    future.
-- ============================================================================
...
begin
  ...
  begin
    if p_event_type = 'finance_payment_confirmed' then
      ...
      if p_payload ->> 'tipe' = 'bahan' and coalesce(p_payload ->> 'admin_email', '') ilike '%endy%' then
        ...
        if v_sender_id is not null then
          select exists(
            ...
            where employee_id = v_sender_id and created_at >= now() - interval '2 hours'
          ) into v_has_recent_photo;
        end if;
      end if;

      v_kc_title := 'Pengajuan Baru Menunggu Verifikasi — ' || coalesce(p_payload ->> 'proyek_nama', p_payload ->> 'proyek', '-');
      v_kc_body :=
        coalesce(v_items_text, '🧾 Item: ' || coalesce(p_payload ->> 'item', '-'))
          ...
          || case when not v_has_recent_photo then
               E'\n\n⚠️ RAWAN: belum ada foto nota/bukti belanja yang diterima dari pengaju untuk pengajuan ini -- mohon dicek sebelum diputuskan.'
             else '' end
          || E'\n\n🔗 Verifikasi di sini: ' || coalesce(p_payload ->> 'verification_link', 'https://finance.haluoleo.id/verifikasi.html');
      v_kc_link := coalesce(p_payload ->> 'verification_link', 'https://finance.haluoleo.id/verifikasi.html');
      v_kc_metadata := jsonb_build_object(
      ...
      if v_branch_id is not null then
        ...
        loop
          ...
          values (v_admin.id, 'system', 'finance_expense_pending_verification', v_kc_title, v_kc_body, v_kc_link, v_kc_metadata);
        end loop;

        -- Only track this for the stale-reminder cron if it isn't ALREADY
        -- approved -- the finance_expense_approved event for this same
        -- pengajuan can (and, in the KK-359 incident, did) arrive first via
        -- the shared outbound queue. finance_pending_transfers existing for
        -- this pengajuan_id is the reliable, order-independent signal that
        -- a decision already happened.
        if (p_payload ->> 'pengajuan_id') is not null then
          select exists(
            select 1 from public.finance_pending_transfers
            where pengajuan_id = (p_payload ->> 'pengajuan_id')::bigint
          ) into v_already_approved;

          if not v_already_approved then
            insert into public.pengajuan_verification_reminders (pengajuan_id, branch_id, title, body, link, metadata)
            values ((p_payload ->> 'pengajuan_id')::bigint, v_branch_id, v_kc_title, v_kc_body, v_kc_link, v_kc_metadata)
            on conflict (pengajuan_id) do update set
              branch_id = excluded.branch_id, title = excluded.title, body = excluded.body,
              link = excluded.link, metadata = excluded.metadata, last_reminded_at = null;
          end if;
        end if;
      end if;

      if not v_has_recent_photo and v_sender_id is not null then
        insert into public.mkc_notifications (user_id, type, category, title, body, metadata)
        values (
          v_sender_id, 'system', 'material_purchase_missing_photo',
          ...
          '🧾 Anda mengajukan pembelian: ' || coalesce(p_payload ->> 'item', '-')
            ...
            || E'\n\n📸 Belum ada foto bahan yang diterima sebagai bukti/alasan pemesanan ini. Pengajuan ini sudah diteruskan ke Kepala Cabang, tapi ditandai rawan karena belum ada bukti fotonya -- tolong tetap kirim foto nota/bahannya ya.'
      ...
      if p_payload ->> 'tipe' in ('bahan', 'tukang') and (p_payload ->> 'pengajuan_id') is not null then
        insert into public.finance_pending_transfers (pengajuan_id, proyek, tipe, branch_id, party_name, reference_no, nominal, admin_email, rekening_tujuan)
        values (
          ...
          p_payload ->> 'keterangan'
        );
      end if;

      if (p_payload ->> 'pengajuan_id') is not null then
        delete from public.pengajuan_verification_reminders where pengajuan_id = (p_payload ->> 'pengajuan_id')::bigint;
      end if;

      v_target_ref := p_payload ->> 'pengajuan_id';

    elsif p_event_type = 'finance_expense_rejected' then
      if (p_payload ->> 'pengajuan_id') is not null then
        delete from public.pengajuan_verification_reminders where pengajuan_id = (p_payload ->> 'pengajuan_id')::bigint;

        update public.finance_pending_transfers
        set rejected_at = now(),
            rejected_reason = 'Pengajuan ditolak di mkh-properti setelah sempat disetujui'
              || case when coalesce(p_payload ->> 'rejected_by', '') <> '' then ' (oleh ' || (p_payload ->> 'rejected_by') || ')' else '' end
        where pengajuan_id = (p_payload ->> 'pengajuan_id')::bigint
    ...
    elsif p_event_type = 'finance_expense_duplicate_rejected' then
      select id into v_branch_id from public.branches where lower(name) = lower(p_payload ->> 'branch_name') limit 1;
      ...
      if v_branch_id is not null then
        ...
        loop
          insert into public.mkc_notifications (user_id, type, category, title, body, link, metadata)
          values (
            v_admin.id, 'system', 'finance_expense_duplicate_rejected',
            'Pengajuan Ditolak Otomatis (Duplikat) — ' || coalesce(p_payload ->> 'proyek_nama', p_payload ->> 'proyek', '-'),
            '🚫 Sistem otomatis menolak pengajuan ini karena item dan nominalnya sama persis dengan pengajuan #' || coalesce(p_payload ->> 'duplicate_of_pengajuan_id', '-') || ' yang sudah disetujui sebelumnya.'
              || E'\n📝 ' || coalesce(p_payload ->> 'keterangan', '-')
              || E'\n💰 Nilai: Rp ' || to_char(coalesce((p_payload ->> 'nominal')::numeric, 0), 'FM999,999,999,999')
              || E'\n👤 Diajukan oleh: ' || coalesce(p_payload ->> 'admin_email', '-')
              || E'\n\nKalau ini SALAH deteksi (bukan pembelian yang sama), tolong minta diajukan ulang manual.',
            '/hr/finance-sync',
            jsonb_build_object('pengajuan_id', p_payload ->> 'pengajuan_id', 'proyek', p_payload ->> 'proyek')
          );
        end loop;
      ...
      v_submitter_name := (regexp_match(coalesce(p_payload ->> 'admin_email', ''), '^(?:Pengawas|Pelapor|Kontraktor)\s*:\s*(.+)$', 'i'))[1];
      v_submitter_phone := null;
      if v_submitter_name is not null then
        select em.phone into v_submitter_phone
        from public.employees em
        where em.full_name ilike '%' || v_submitter_name || '%' and em.deleted_at is null
          and em.employment_status = 'active' and em.phone is not null
        limit 1;

        if v_submitter_phone is null then
          select cs.phone into v_submitter_phone
          from public.contractor_wa_senders cs
          where cs.full_name ilike '%' || v_submitter_name || '%' and cs.phone is not null
          limit 1;
        end if;
      end if;

      if v_submitter_phone is not null then
        perform public.automation_post(
          '/api/admin/send-wa-message',
          jsonb_build_object(
            'phone', v_submitter_phone,
            'message', '🚫 Pengajuan Anda otomatis ditolak sistem karena item dan nominalnya sama persis dengan pengajuan yang sudah disetujui sebelumnya:'
              || E'\n📝 ' || coalesce(p_payload ->> 'keterangan', '-')
              || E'\n💰 Rp ' || to_char(coalesce((p_payload ->> 'nominal')::numeric, 0), 'FM999,999,999,999')
              || E'\n\nKalau ini bukan pembelian yang sama (bukan duplikat), tolong hubungi Kepala Cabang atau Super Admin untuk diajukan ulang.'
          ),
          8000
        );
      end if;
...
create or replace function public.remind_stale_pengajuan_verification()
returns void
language plpgsql
security definer
...
begin
  for v_row in
    select * from public.pengajuan_verification_reminders
    where created_at <= now() - interval '2 hours'
      and (last_reminded_at is null or last_reminded_at <= now() - interval '2 hours')
  loop
    -- Self-heal: a decision (approve) can have raced this row into
    -- existence after the fact (see 0261) -- finance_pending_transfers
    -- existing for this pengajuan_id is the reliable signal it's already
    -- resolved, so stop nagging about it instead of re-sending forever.
    select exists(
      select 1 from public.finance_pending_transfers
      where pengajuan_id = v_row.pengajuan_id
    ) into v_already_decided;

    if v_already_decided then
      delete from public.pengajuan_verification_reminders where pengajuan_id = v_row.pengajuan_id;
      continue;
    end if;
    ...
    if v_row.branch_id is not null then
      for v_kc in
        select em.id from public.employees em
        join public.roles r on r.id = em.role_id
        where em.branch_id = v_row.branch_id and em.deleted_at is null and em.employment_status = 'active' and r.key = 'kepala_cabang'
      loop
        insert into public.mkc_notifications (user_id, type, category, title, body, link, metadata)
        values (
          v_kc.id, 'system', 'finance_expense_pending_verification',
          '🔔 Pengingat: ' || v_row.title,
          v_row.body || E'\n\n(pengingat -- pengajuan ini belum diputuskan sejak ' || to_char(v_row.created_at, 'DD Mon HH24:MI') || ')',
          v_row.link, v_row.metadata
        );
      end loop;
    end if;
    update public.pengajuan_verification_reminders set last_reminded_at = now() where pengajuan_id = v_row.pengajuan_id;
  end loop;
end;
$function$;

-- One-off cleanup: pengajuan 359 ("Cat exterior 1 pail") was approved and
-- paid on 2026-09-17 but its reminder row survived the race described
-- above and has been spamming every ~2 hours since. The self-healing
-- cron above will also catch this on its own next run, but clear it now
-- so the immediate spam stops.
delete from public.pengajuan_verification_reminders
where pengajuan_id in (
  select pengajuan_id from public.pengajuan_verification_reminders pvr
  where exists (
    select 1 from public.finance_pending_transfers fpt
    where fpt.pengajuan_id = pvr.pengajuan_id
  )
);
