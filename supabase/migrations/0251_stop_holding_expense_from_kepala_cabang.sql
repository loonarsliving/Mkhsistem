-- Owner's explicit reversal of 0244 ("hold Endy's bahan pengajuan from
-- Vando until nota photo is sent"): "km jangan selalu seperti itu biarkan
-- vando yg putuskan, cukup sampaikan vando kalau itu rawan karna dia tidak
-- menyampaikan bukti nota" -- Vando should always get every pengajuan
-- immediately and decide for himself, not have it silently withheld.
-- Instead, the same "no nota photo in the last 2 hours" signal 0244 already
-- computes now gets attached to Vando's own notification as an explicit
-- risk warning, so he can weigh it when deciding instead of never seeing
-- the pengajuan at all.
--
-- pending_expense_approval_notifications / release_pending_expense_approval_notifications
-- (0244) are left in place (harmless, unused going forward) rather than
-- dropped -- no live rows depend on this path continuing to write into it.
...
begin
  ...
  begin
    if p_event_type = 'finance_payment_confirmed' then
      ...
      -- Owner's reversal (this migration): the "no recent nota photo" signal
      -- is now a warning attached to Vando's own notification, never a
