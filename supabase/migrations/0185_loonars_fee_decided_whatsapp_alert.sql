-- ============================================================================
-- MK Connect — 0185: loonars fee-claim decision alert (companion to MKH
-- Property's 0014_loonars_fee_approval_moves_to_cfo.sql)
--
-- Fee-claim approval moved fully to CFO on MKH Property's /verifikasi.html
-- (no more separate owner-approval step in loonars-sales). Approving or
-- rejecting the fee there now enqueues a new outbound loonars_fee_decided
-- event; this migration adds the inbound handling: alert every super_admin,
-- reusing the existing loonars_fee_alert category (already whitelisted for
-- the WhatsApp relay in 0098), so the CFO's decision -- approved or
-- rejected -- reaches super_admin over WhatsApp instead of the CFO page's
-- old internal Telegram message.
--
-- CREATE OR REPLACE with the full live body (queried directly from
-- production) plus the new branch -- see 0175's header for why this
-- matters.
-- ============================================================================
