-- ============================================================================
-- MK Connect — 0255: Fix finance_expense_submitted crashing for non-Endy admins
--
-- Real incident (2026-09-09): pengajuan 321 (Anang, admin_email
-- "Kontraktor: Anang") failed to sync at all -- sync_log shows
-- error 55000 "record \"v_sender\" is not assigned yet". Root cause:
-- v_sender was declared as a bare `record`, only ever assigned inside
-- `if p_payload->>'tipe' = 'bahan' and admin_email ilike '%endy%'`. For
-- ANY other admin_email (Anang, Rebecca, Muhammad Syafiq, ...) that branch
-- is skipped, leaving v_sender structurally unassigned -- and the later
-- unconditional read `v_sender.id is not null` throws immediately on an
-- unassigned record, regardless of `not v_has_recent_photo` supposedly
-- short-circuiting first.
--
-- Because sync_inbound's exception handler rolls back everything done
-- inside its `begin...exception` block, this crash silently discarded the
-- Kepala Cabang verification notification AND the
-- pengajuan_verification_reminders row for every bahan/tukang pengajuan
-- submitted by anyone other than Endy -- exactly the safety net 0252 was
-- built to guarantee. Only surfaced now because a Super Admin manually
-- checked sync_log after noticing a pengajuan that should have reached
-- Kepala Cabang for verification.
--
-- Fix: replace the bare `record` with a plain `uuid` variable, which is
-- simply null until assigned -- no "unassigned record" failure mode
-- possible.
-- ============================================================================
