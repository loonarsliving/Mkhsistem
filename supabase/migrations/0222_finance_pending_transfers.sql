-- Owner's request: bukti transfer (sent by Super Admin over WhatsApp after
-- Kepala Cabang approves a gaji tukang / pembelian bahan pengajuan) should
-- be the real final approval that posts jurnal and deducts kas in
-- mkh-properti -- not Kepala Cabang's approval alone, which today posts
-- jurnal before any money has actually moved. The bukti transfer photo
-- itself must still reach Kepala Cabang + Endy so they can forward it to
-- the tukang/toko.
--
-- This is the MK Connect side of that flow:
--   1. finance_expense_approved (already existed) now ALSO queues a row
--      here -- the "awaiting transfer proof" list.
--   2. Super Admin sends the bukti transfer photo over WhatsApp -- handled
--      by lib/ai/domains/transfer-proof-confirmation.ts (FIFO-picks the
--      oldest unconfirmed row here, same accepted trade-off as the
--      existing loonars-fee-approval.ts WA-reply pattern).
--   3. AI reads the photo (lib/ai/domains/transfer-proof-recognition.ts),
--      the photo is forwarded to Kepala Cabang + Endy, and a
--      finance_expense_transfer_confirmed event is enqueued in this DB's
--      own sync_log (direction='outbound') -- delivered to mkh-properti's
--      sync_inbound by the existing sync_dispatch_pending cron, where
--      (0018, mkh-properti) it finally posts the jurnal double-entry.
create table public.finance_pending_transfers (
  id uuid primary key default gen_random_uuid(),
  pengajuan_id bigint not null,
  proyek text not null,
  tipe text not null check (tipe in ('bahan', 'tukang')),
...
create index finance_pending_transfers_unconfirmed_idx on public.finance_pending_transfers (created_at) where confirmed_at is null;

alter table public.finance_pending_transfers enable row level security;
create policy finance_pending_transfers_select on public.finance_pending_transfers for select
  using (public.app_has_permission('construction_finance.manage'));

-- CREATE OR REPLACE of the full sync_inbound function (only the
-- finance_expense_approved branch changed -- queues the pending-transfer
-- row in addition to the existing super_admin WA notification -- every
-- other branch copied verbatim from the live definition).
      ...
      loop
        ...
        values (
          ...
          '🧾 Item: ' || coalesce(p_payload ->> 'item', '-')
            ...
            || E'\n\n💸 Setelah transfer, kirim foto bukti transfer ke WhatsApp ini -- akan otomatis dibaca AI, dicatat ke jurnal, dan diteruskan ke Kepala Cabang + Endy.',
          '/hr/finance-sync',
          jsonb_build_object('pengajuan_id', p_payload ->> 'pengajuan_id', 'proyek', p_payload ->> 'proyek')
        );
      end loop;

      -- Queue this pengajuan as awaiting transfer proof (0222) -- only for
      -- the two human-submitted types this flow covers (bahan/tukang);
      -- other approved-event-shaped payloads never reach this branch since
      -- mkh-properti's 0006/0008 triggers already scope to tipe in
      -- ('bahan', 'tukang') before dispatching finance_expense_approved.
      select id into v_branch_id from public.branches where lower(name) = lower(p_payload ->> 'branch_name') limit 1;
