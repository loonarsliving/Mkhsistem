-- ============================================================================
-- MK Connect — 0259: Handle finance_expense_duplicate_rejected from MK Property
--
-- Companion to mkh-properti's 0035 (trg_pengajuan_expense_duplicate_guard),
-- itself a direct owner request (2026-09-11) after 4 real duplicate
-- reimbursement incidents this week: from now on a detected duplicate
-- should be rejected immediately and both Kepala Cabang AND the original
-- submitter told why -- not just flagged as "tidak cocok" on a bukti
-- transfer reply after the money has already moved (the KK-316/325
-- incident: paid twice before anyone caught it).
--
-- Kepala Cabang is a real employee, so a normal mkc_notifications insert
-- (WA-forwarded via the allowlist below) reaches them. The original
-- submitter is very often an external contractor (e.g. Anang) who has no
-- employees row at all -- mkc_notifications can't target them, so this
-- resolves their phone the same way resolveRecipients
-- (lib/ai/domains/transfer-proof-confirmation.ts) does: strip the
-- "Pengawas:"/"Pelapor:"/"Kontraktor:" label off admin_email, look up
-- employees first, then contractor_wa_senders, and send a direct WhatsApp
-- text via the existing /api/admin/send-wa-message utility.
-- ============================================================================
...
begin
  if new.category = any (array[
    ...
    'finance_expense_duplicate_rejected'
  ]) then
