-- Owner's ask, after two confirmed real incidents this session (Vando never
-- seeing a pengajuan on his verification queue despite WhatsApp reporting
-- success; Super Admin's own "Pengeluaran Disetujui" alert reporting
-- success but never arriving): "bikin kode untuk memastikan tidak ada pesan
-- yg tertinggal lagi". Whacenter's /send API only confirms the message was
-- ACCEPTED into its queue, never that it was actually delivered -- there is
-- no delivery-receipt webhook this system can rely on to detect a silent
-- drop after the fact. The only reliable mitigation available is a
-- time-based safety net: track what's still genuinely unresolved and
-- automatically remind about it, so a single silently-dropped WhatsApp
-- send is never the only chance a message had to get through.
--
-- Two tracked gaps:
--   1. Kepala Cabang verification (finance_expense_pending_verification) --
--      a pengajuan that's neither been approved nor rejected after 2 hours
--      gets reminded. Requires 0031 (mkh-properti) to have added a
--      finance_expense_rejected sync event -- without a rejection signal, a
--      REJECTED pengajuan would look identical to a silently-missed one and
--      get reminded forever.
--   2. Super Admin transfer (finance_expense_alert) -- a
--      finance_pending_transfers row still neither confirmed nor rejected
--      after 3 hours gets reminded.
-- Both are debounced (a fresh reminder only after the previous one is 2-3
-- hours old) so this never spams -- it only ever catches something that has
-- sat untouched for a real stretch of time.

create table public.pengajuan_verification_reminders (
  pengajuan_id bigint primary key,
  branch_id uuid,
  title text not null,
  body text not null,
  link text,
...
alter table public.finance_pending_transfers add column last_reminded_at timestamptz;
  ...
  begin
    if p_event_type = 'finance_payment_confirmed' then
      ...
      if v_branch_id is not null then
        ...
        -- This migration: track so remind_stale_pengajuan_verification() can
        -- follow up if neither an approval nor a rejection ever arrives.
      ...
      if not v_has_recent_photo and v_sender.id is not null then
      ...
      -- This migration: the pengajuan was decided (approved) -- no longer
      -- needs a Kepala Cabang verification reminder.
      if (p_payload ->> 'pengajuan_id') is not null then
      ...
      -- This migration: mkh-properti's 0031. The pengajuan was decided
      -- (rejected) -- no longer needs a Kepala Cabang verification
      -- reminder. Nothing else to do: mkh-properti's own UI already shows
      -- the rejection to whoever submitted it.
      if (p_payload ->> 'pengajuan_id') is not null then
        delete from public.pengajuan_verification_reminders where pengajuan_id = (p_payload ->> 'pengajuan_id')::bigint;
      end if;
      v_target_ref := p_payload ->> 'pengajuan_id';
...
-- ----------------------------------------------------------------------------
-- remind_stale_pengajuan_verification(): every 30 min, catches any pengajuan
-- still awaiting a Kepala Cabang decision 2+ hours after the original
-- notification (and not reminded in the last 2 hours) and resends it.
-- ----------------------------------------------------------------------------
...
comment on function public.remind_stale_pengajuan_verification is
  'Every 30 min: resends a Kepala Cabang verification notification for any pengajuan still undecided 2+ hours after it was first sent -- a safety net for a WhatsApp send Whacenter reported successful but that never actually arrived.';

select cron.schedule('remind-stale-pengajuan-verification', '*/30 * * * *', $$select public.remind_stale_pengajuan_verification();$$);

-- ----------------------------------------------------------------------------
-- remind_stale_pending_transfers(): every 30 min, catches any
-- finance_pending_transfers row still neither confirmed nor rejected 3+
-- hours after Vando's approval (and not reminded in the last 3 hours) and
-- sends Super Admin a consolidated reminder.
-- ----------------------------------------------------------------------------
create or replace function public.remind_stale_pending_transfers()
returns void
language plpgsql
security definer
set search_path = public
...
begin
  for v_row in
    select id, pengajuan_id, party_name, nominal, created_at
    from public.finance_pending_transfers
    where confirmed_at is null and rejected_at is null
      and created_at <= now() - interval '3 hours'
      and (last_reminded_at is null or last_reminded_at <= now() - interval '3 hours')
    order by created_at
  loop
    v_count := v_count + 1;
    v_ids := v_ids || v_row.id;
    v_lines := v_lines || E'\n' || v_count || '. ' || coalesce(v_row.party_name, 'Pengajuan') || ' — Rp ' || to_char(v_row.nominal, 'FM999,999,999,999');
  end loop;
  ...
  loop
    ...
    values (
      v_admin.id, 'system', 'finance_expense_alert',
      '🔔 Pengingat: ' || v_count || ' Pengeluaran Menunggu Transfer',
      'Pengajuan berikut sudah disetujui tapi belum ada bukti transfer:' || v_lines
        || E'\n\nKirim foto bukti transfer ke WhatsApp ini untuk masing-masing.',
      '/hr/finance-sync'
    );
  end loop;

  update public.finance_pending_transfers set last_reminded_at = now() where id = any (v_ids);
end;
$$;

comment on function public.remind_stale_pending_transfers is
  'Every 30 min: reminds every Super Admin about any approved expense still awaiting a transfer 3+ hours later -- a safety net for a WhatsApp send Whacenter reported successful but that never actually arrived.';

select cron.schedule('remind-stale-pending-transfers', '*/30 * * * *', $$select public.remind_stale_pending_transfers();$$);
