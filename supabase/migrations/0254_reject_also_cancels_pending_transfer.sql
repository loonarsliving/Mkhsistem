-- ============================================================================
-- MK Connect — 0254: finance_expense_rejected also cancels a pending transfer
--
-- Real incident (2026-09-09): Vando/Endy manually submitted 3 lump-sum
-- reimbursement pengajuan for Anang ("belanja 7 sept") that got approved and
-- queued in finance_pending_transfers, BEFORE Anang's actual nota photos were
-- processed into 3 separate, itemized pengajuan for the exact same purchases
-- (KK-317/318/319, two of them identical to the rupiah). Once the duplicate
-- manual ones were corrected to 'rejected' in mkh-properti, the resulting
-- finance_expense_rejected sync event only cleaned up
-- pengajuan_verification_reminders -- it never touched the
-- finance_pending_transfers row that the earlier finance_expense_approved
-- event had already created, so Super Admin could still have paid the
-- rejected duplicate from the pending-transfer queue. Had to be caught and
-- fixed by hand this time; this closes the gap so it can't happen silently
-- again for any future approve-then-correct-to-rejected pengajuan.
-- ============================================================================
  ...
  begin
    if p_event_type = 'finance_payment_confirmed' then
      ...
      if (p_payload ->> 'pengajuan_id') is not null then
        ...
        -- An approved pengajuan can still be corrected to 'rejected' later
        -- (e.g. a duplicate caught after the fact) -- if that already queued
        -- a finance_pending_transfers row, it must be pulled from the payable
        -- queue too, or Super Admin could still transfer money for something
