-- Cosmetic follow-up to the bukti-transfer submitter-routing fix
-- (lib/ai/domains/transfer-proof-confirmation.ts): the bukti transfer photo
-- can now forward to whichever employee actually submitted the pengajuan
-- (Endy or Rebecca, per mkh-properti's "Pengawas"/"Pelapor" label), not
-- always Endy -- so the WA notification text should say so generically
-- instead of hardcoding his name.
--
-- CREATE OR REPLACE below is the full live definition of sync_inbound
-- (this project's own, handling inbound events from mkh-properti), with
-- only that one string changed in the finance_expense_approved branch.
-- Every other branch is copied verbatim.
      ...
      loop
        ...
        values (
          ...
          '🧾 Item: ' || coalesce(p_payload ->> 'item', '-')
            ...
            -- This migration (0233): was hardcoded "Kepala Cabang + Endy".
