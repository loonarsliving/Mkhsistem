-- Owner's ask: the WhatsApp alert a branch's Kepala Cabang gets when Finance
-- (e.g. Endy) submits a gaji tukang pengajuan (via the mkh-properti app,
-- relayed here through sync_inbound's finance_expense_submitted handler)
-- should show the remaining borongan contract balance ("sisa gaji"), not
-- just the amount being requested right now. mkh-properti's 0017 migration
-- now forwards sisa_kontrak/terbayar_baru in the payload for tipe='tukang'
-- -- this surfaces those in the WA message body, and threads tipe/branch_id/
-- the raw numbers through mkc_notifications.metadata so the WhatsApp relay
-- (app/api/ai/whatsapp-relay/route.ts) can add an AI-computed payment
-- recommendation on top, cross-referencing recent photo-progress evidence
-- for that branch.
--
-- CREATE OR REPLACE of the full sync_inbound function (only the
-- finance_expense_submitted branch's WA body/metadata changed) -- this
-- function handles many unrelated event types, all copied through verbatim
-- from the live definition to avoid drift.
