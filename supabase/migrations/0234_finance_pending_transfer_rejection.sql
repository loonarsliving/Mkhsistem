-- ============================================================================
-- MK Connect — 0234: Super Admin can reject an approved-but-unpaid pengajuan
--
-- Owner's ask: once a Kepala Cabang approves gaji tukang/pembelian bahan,
-- there was no way to catch a data-entry mistake (e.g. wrong blok) before
-- money moves -- the only path was sending a bukti transfer anyway, or a
-- manual database fix. Super Admin can now reply "TOLAK <kode> <alasan>"
-- on WhatsApp (lib/ai/domains/transfer-rejection.ts) instead of a bukti
-- transfer photo.
--
-- rejected_at/rejected_by mirror confirmed_at/confirmed_by for the same
-- race-safe-claim pattern; rejected_reason stores the free-text reason.
-- Both the confirm and reject flows now also filter out already-rejected
-- rows from the pending queue.
--
-- The approval WA text now includes the pengajuan's reference code (KK-xxx)
-- so Super Admin has something to quote in a TOLAK reply -- previously
-- nothing in the notification identified which pengajuan was which beyond
-- free text.
-- ============================================================================

alter table public.finance_pending_transfers add column rejected_at timestamptz;
alter table public.finance_pending_transfers add column rejected_by uuid references public.employees(id);
alter table public.finance_pending_transfers add column rejected_reason text;
