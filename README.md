...
- **FRIDAY — Executive Intelligence Layer** (`/friday`) — lapisan analisa di atas seluruh modul. Setiap modul lain menjawab pertanyaan di dalam satu domain; FRIDAY membaca CRM, iklan Meta, keuangan cabang, HR/absensi, Markom, okupansi, dan kesehatan automation **sekaligus** lalu menghasilkan satu briefing eksekutif: situasi → analisa akar penyebab → risiko bila tidak bertindak → minimal 3 alternatif solusi beserta kerugian masing-masing → rekomendasi + alasan mengalahkan alternatif → hasil yang diharapkan. Angka lintas domain (mis. biaya iklan per lead, porsi prospek mangkrak) dihitung di kode, bukan oleh model — lihat `lib/ai/friday/signals.ts`. Berjalan otomatis tiap hari 06:30 WITA, dan bisa diminta on-demand dengan pertanyaan spesifik.
  ...
  **Holding Architecture** (`/friday/holding`) — FRIDAY membaca seluruh grup, bukan hanya satu perusahaan. Filosofinya: **setiap bisnis berdiri sendiri**. Villa, Homestay, Kos, Hotel, Coffee Shop, dan bisnis masa depan masing-masing punya dashboard, database, automation, user, dan **berhak memilih sistem operasionalnya sendiri**. Source of truth tetap di sistem masing-masing — FRIDAY membacanya lewat **Connector**, dan tidak menggantikannya.

  Connector hanya punya empat kata kerja: ambil data, kirim data, jalankan automation, ambil status. **Tanpa logika bisnis** — begitu connector mulai memutuskan arti sebuah angka, source of truth diam-diam pindah ke FRIDAY dan kemandirian itu hilang. Dua jenis tersedia: `internal_mkh` (unit Developer, membaca database MK Connect langsung) dan `http` (dashboard bisnis lain yang menyajikan JSON).

  Snapshot tiap bisnis punya dua kanal: `metrics[]` dengan kosakata terbatas yang sama untuk semua bisnis (sehingga perbandingan antar unit jadi aritmetika, bukan tafsiran) dan `narrative` berisi detail domain bisnis itu sendiri apa adanya. Perbandingan berjalan di kanal pertama, penjelasan di kanal kedua — **Brain tidak perlu mengenal domain baru**. Menambah bisnis ke-100 adalah satu baris di `holding_businesses`, bukan perubahan kode.

  **Satu bisnis yang tidak terbaca bukan bisnis yang sepi.** Connector yang gagal menghasilkan status `unreachable`, tidak pernah angka nol, dan `holding.ts` **memaksa** penilaian unit tersebut menjadi `data_tidak_tersedia` apa pun yang ditulis model — menyebut unit "kritis" karena dashboard-nya mati adalah kesalahan yang akan menghancurkan kepercayaan Direksi pada laporan grup. Satu unit mati menurunkan kualitas briefing, tidak pernah membatalkannya.
...
- **Scheduled jobs** — `pg_cron` aktif di database live dengan **56 job terjadwal** — absensi, CRM, Markom, konten, iklan Meta, knowledge bank, sinkronisasi finance (3 di antaranya sengaja dinonaktifkan). Inventaris lengkapnya — beserta jadwal UTC/WITA, empat jalur eksekusi otomasi, dan hasil audit lapisan ini — ada di [`docs/AUTOMATION.md`](docs/AUTOMATION.md).
